...
class Cashtippr {
	...
	public function startSession() {
		...
		if ($success !== true) // TODO don't rely on PHP sessions, implement or own using a custom cookie
			static::notifyErrorExt("A session has already been started by another plugin. CashTippr might not work properly. Please check the PHP Notices for details.", '');
	}
	...
	*/
	...
	public function createBlockchainApiInstance(string $implementation = '', string $blockchainApiUrl = ''): AbstractBlockchainApi {
		if (empty($implementation) || empty($blockchainApiUrl)) {
			...
			$blockchainApiUrl = $this->settings->get('bchdApi');
		}
