		...
		<?php
		$this->description( __( 'he BCHD node REST API gateway backend to connect to for the BCH blockchain data. You may run your own backend: https://github.com/simpleledgerinc/bchd', 'ekliptor' ) );
		?>
		<p>
			<label for="<?php $this->fieldId( 'bchdApi' ); ?>" class="ct-toblock">
				<?php esc_html_e( 'URL', 'ekliptor' ); ?>
			</label>
		</p>
		<p class="ct-input-wrap">
			<input type="text" name="<?php $this->fieldName( 'bchdApi' ); ?>" class="large-text" id="<?php $this->fieldId( 'bchdApi' ); ?>" placeholder="<?php echo esc_attr( $apiPlaceholder ); ?>" value="<?php echo esc_attr( $this->getFieldValue( 'bchdApi' ) ); ?>" />
		</p>
		<?php 
