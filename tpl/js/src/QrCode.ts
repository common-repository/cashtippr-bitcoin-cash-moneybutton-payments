...
export class QrCode extends AbstractModule {
    ...
    protected showQrDialog(target: Element) {
        const paymentButtonWrapper = this.cashtippr.$(target).parent().parent().parent().parent() as JQuery<HTMLElement>;
        ...
        if (paymentCtrlWrapper.find("input[name='amount']").length !== 0)
            amount = parseInt(paymentCtrlWrapper.find("input[name='amount']").val() as string); // custom editable amount by user
        let params = {
            txid: txid,
            am: amount
        }
